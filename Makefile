...
test:
	GOARCH=amd64 go test -p 1 ./...

test-mac:
	GOOS=darwin GOARCH=amd64 go test -p 1 ./...

cover:
	GOARCH=amd64 go test -p 1 --coverprofile=coverage.out ./...
...
build: clean mod fmt vet
	go build -ldflags "-s -w" -o $(NAME) cli/fishdb.go

build-mac: clean mod fmt vet
	GOOS=darwin GOARCH=amd64 go build -o $(NAME).mac cli/fishdb.go

build-win: clean mod fmt vet
	GOOS=windows GOARCH=amd64 go build -o $(NAME).exe cli/fishdb.go

build-arm7: clean mod fmt vet
	GOOS=linux GOARCH=arm GOARM=7 go build -o $(NAME).arm7 cli/fishdb.go

build-arm8: clean mod fmt vet
	GOOS=linux GOARCH=arm64 go build -o $(NAME).arm8 cli/fishdb.go
