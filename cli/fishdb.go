...
/*
FishDB is a graph based database which aims to provide a lightweight solution
...
- For more complex queries FishDB has an own query language called EQL with an sql-like syntax.
...
func main() {
	...
	flag.Usage = func() {
		...
		fmt.Println()
		fmt.Println("FishDB graph based database")
		fmt.Println()
		fmt.Println("Available commands:")
		fmt.Println()
		fmt.Println("    console   FishDB server console")
		fmt.Println("    server    Start FishDB server")
		fmt.Println()
...
func RunCliConsole() {
	...
	host := flag.String("host", chost, "Host of the FishDB server")
	port := flag.String("port", cport, "Port of the FishDB server")
	...
	if *cmdfile == "" && *cmdline == "" {
		fmt.Println(fmt.Sprintf("FishDB %v - Console",
	...
	if *cmdfile != "" {
		...
		histfile := filepath.Join(filepath.Dir(os.Args[0]), ".fishdb_console_history")
