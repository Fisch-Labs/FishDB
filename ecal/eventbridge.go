...
/*
EventMapping is a mapping between FishDB event types to FishDB specific event kinds in ECAL.
*/
...
func (eb *EventBridge) Handle(gm *graph.Manager, trans graph.Trans, event int, ed ...interface{}) error {
	...
	if name, ok := EventMapping[event]; ok {
		eventName := fmt.Sprintf("FishDB: %v", name)
		...
		event := engine.NewEvent(fmt.Sprintf("FishDB: %v", name), strings.Split(name, "."), state)
		...
		if err != nil {
			eb.Logger.LogDebug(fmt.Sprintf("FishDB event %v was handled by ECAL and returned: %v", name, err))
		}
	}
