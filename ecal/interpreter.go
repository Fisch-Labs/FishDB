...
import (
	...
	ecalconfig "github.com/Fisch-Labs/ecal/config"
	"github.com/Fisch-Labs/ecal/engine"
	"github.com/Fisch-Labs/ecal/scope"
	"github.com/Fisch-Labs/ecal/stdlib"
...
func (si *ScriptingInterpreter) Run() error {
	...
	if err == nil {
		...
		i.CreateRuntimeProvider("fishdb-runtime")

		// Adding functions

		AddFishDBStdlibFunctions(si.GM)

		// Adding rules

		sockRule := &engine.Rule{
			Name:            "FishDB-websocket-communication-rule", // Name
		...
		if err == nil {
			...
			// FishDB graph events are now forwarded to ECAL via the eventbridge.
...
/*
AddFishDBStdlibFunctions adds FishDB related ECAL stdlib functions.
*/
func AddFishDBStdlibFunctions(gm *graph.Manager) {
	stdlib.AddStdlibPkg("db", "FishDB related functions")
