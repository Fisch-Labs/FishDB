...
`)
	...
	if err := ds.Run(); err == nil || err.Error() != `ECAL error in fishdb-runtime (testscripts/main.ecal): some error () (Line:2 Pos:1)
...
FishDB: db.node.store -> mysink : ECAL error in fishdb-runtime (testscripts/main.ecal): Oh no () (Line:8 Pos:7))` {
		t.Error("Unexpected result:", err)
		return
	}
	...
	if err == nil || err.Error() != `GraphError: Graph rule error (Taskerror:
FishDB: db.edge.store -> mysink : ECAL error in fishdb-runtime (testscripts/main.ecal): Oh no edge () (Line:15 Pos:7))` {
		t.Error("Unexpected result:", err)
		return
	}
	...
	if err == nil || err.Error() != `GraphError: Graph rule error (Taskerror:
FishDB: db.edge.created -> mysink : ECAL error in fishdb-runtime (testscripts/main.ecal): Oh no edge2 () (Line:18 Pos:7))` {
		t.Error("Unexpected result:", err)
		return
	}
	...
	if err == nil || err.Error() != `GraphError: Graph rule error (Taskerror:
FishDB: db.edge.updated -> mysink : ECAL error in fishdb-runtime (testscripts/main.ecal): Oh no edge3 () (Line:21 Pos:7))` {
		t.Error("Unexpected result:", err)
		return
	}
