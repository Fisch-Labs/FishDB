...
/*
StartServer runs the FishDB server. The server uses config.Config for all its configuration
parameters.
*/
func StartServer() {
...
/*
StartServerWithSingleOp runs the FishDB server. If the singleOperation function is
...
func StartServerWithSingleOp(singleOperation func(*graph.Manager) bool) {
	...
	print(fmt.Sprintf("FishDB %v", config.ProductVersion))
	...
	// Register FishDB API endpoints - depending on if access control has been enabled
